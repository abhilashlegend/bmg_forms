jQuery($ => {
  const fbEditor = document.getElementById("cupid-forms-build-wrap");
  var options = {
  ...
  function generateForm() {
      formName = document.getElementById('cupid-form-name').value;
      ...
      if(formName !== "" && formData.length > 2){
        ...
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            ...
            window.location.replace("admin.php?page=cupid-forms");
          } 
          };
      xhttp.open("POST", "admin-ajax.php?action=cupid_generate_form",true);
