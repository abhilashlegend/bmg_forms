...
$result = $wpdb->get_results("SELECT * FROM $table_name ORDER BY id");
		$items = $wpdb->num_rows;
					...
					if($items > 0 && $result) {
						...
						$p->target("admin.php?page=cupid-forms"); 
				...
				if(isset($_POST['delete'])) {
					...
					foreach($row_array as $f) {
						...
						$form_table = $wpdb->prefix . 'cupid_forms_' . $form_name . $f;
...
<div class="wrap">
	...
	<?php } ?>
<form method="post" name="cupid_frm">
<div class="tablenav top">
	...
	foreach($result as $row) {
			...
			<tr>
				...
				<td>
					[cupid_forms <?php echo 'Id="' . $row->id . '"'; ?>]
					<a href="<?php  echo esc_html( admin_url('admin.php?page=cupid_mail_config&form_id=' ) ) . $row->id; ?>" class="btn btn-primary btn-sm pull-right">Mail Config</a>
				</td>
